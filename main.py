...
logging.basicConfig(filename="./log/run_scheduled_webscraper.log", encoding="utf-8")


@scheduler.task(
    "cron", id="running_webscraper", day_of_week="mon", hour="00", minute="05"
)
def run_webscraper():
    logger.info(f"Starting scheduled job: Webscraping engine at {datetime.now()}")
...
def inject_context():
    ...
    return {"selected_day": day, "selected_city": city, "lang": lang}
