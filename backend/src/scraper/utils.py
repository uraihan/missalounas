...
from src.shared.config import DATE_FORMAT


def unpickled_city_dict(city_data):
    return [namedtuple("City", data.keys())(**data) for data in city_data]


def combine_restaurants(weekly_menu):
    """Combine duplicate restaurant entries (like Newton's json entry).
    Params:
    ...
    return [
        {"restaurant_name": name, "area": list(area_set)[0], "menu_options": option}
        for name, option in combined_menu.items()
    ]


def format_date(original_date: str, response_format: str, year=None) -> date:
    date_format = DATE_FORMAT
    try:
        datetime_fmt = datetime.strptime(str(original_date), response_format).date()
        if year:
...
def create_empty_item(restaurant_name, area_name, lang):
    ...
    no_food = unified_json.IndividualMenu(
        food_name=None, diets=None, menu_type=None, date=None, menu_uid=None, lang=lang
    )

    return asdict(
        unified_json.RestaurantContainer(
            restaurant_name=restaurant_name, area=area_name, menu_options=[no_food]
        )
    )
