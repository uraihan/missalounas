...
from src.scraper import config, utils
from src.scraper.parsers.base import GenericRestaurantParser
...
class UnicafeParser(GenericRestaurantParser):
    def _get_essential_chunk(self, response, rest_id):
    ...
    def handle(
        ...
        for idx, lang in enumerate(config.SUPPORTED_LANGS):  # ['en', 'fi']
            ...
            response_json = self._get_essential_chunk(response_json, restaurant_id[idx])

            for data in response_json.menuData.menus:
                date = utils.format_date(data.date, date_format, year=current_year)
                day_menu = []
                for item in data.data:
                    ...
                    menu_item = unified_json.IndividualMenu(
                        food_name, food_diet, menu_type, date, menu_uid, lang
                    )
